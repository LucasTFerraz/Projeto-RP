...
class ResNet20(nn.Module):
    def __init__(self, num_class=10, l_features = 512):
        ...
        if num_class == 200:
            self.fc13 = nn.Sequential(nn.Linear(l_features * 2 * 2, l_features*8),
                                      nn.ReLU(inplace=True))
            #self.fc13 = nn.Sequential(nn.Linear(l_features * 2 * 2, 4096),nn.ReLU(inplace=True))
        else:
            self.fc13 = nn.Sequential(nn.Linear(l_features * 1 * 1, l_features*8),
                                      nn.ReLU(inplace=True))
        self.linear = nn.Linear(l_features*8, num_class, bias=False)
