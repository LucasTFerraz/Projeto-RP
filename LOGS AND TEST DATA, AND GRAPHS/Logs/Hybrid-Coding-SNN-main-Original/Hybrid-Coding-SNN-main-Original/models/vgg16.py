...
class VGG16(nn.Module):
    def __init__(self, num_class, dropout=0, l_features = 512):
        ...
        if num_class == 1000:
            self.fc14 = nn.Sequential(nn.Linear(7 * 7 * l_features, l_features*8, bias=self.bias),
                                      nn.ReLU(inplace=True) )
        elif num_class == 200:
            self.fc14 = nn.Sequential(nn.Linear(2 * 2 * l_features, l_features*8, bias=self.bias),
                                      nn.ReLU(inplace=True) )
        else:
            self.fc14 = nn.Sequential(nn.Linear(1 * 1 * l_features, l_features*8, bias=self.bias),
                                      nn.ReLU(inplace=True) )

        self.fc15 = nn.Sequential(nn.Linear(l_features*8, l_features*8, bias=self.bias),
                                  nn.ReLU(inplace=True))
        self.fc16 = nn.Sequential(nn.Linear(l_features*8, num_class, bias=self.bias),)
